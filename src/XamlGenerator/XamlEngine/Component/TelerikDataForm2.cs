...
namespace XamlGenerator.Component
{
    ...
    //</Grid>
        ...
        #endregion
        #region Constructor
        public TelerikDataForm2(string EntityTypeName, string EntitySetName, IEnumerable<EntityMetaData> MetaDatas, ref IDataSourceManager DomainDataSourceManager)
        {
        ...
        public void CreateDataFormFromMetaData()
        {
            ...
            xaml = string.Format(DataFormDefinition, DataFormName, TemplateName, TemplateName, TemplateName, GetDataSourceName()); //each mode :read only, edit , new item can have different template.
        ...
        private string GetColumns(IEnumerable<EntityMetaData> MetaDatas)
        {
            ...
            string TextBlockTemplate = "<TextBlock Text=\"{{Binding {0} }}\"  Grid.Row=\"{1}\" Grid.Column=\"{2}\" />\n";
            string TextBoxTemplate = "<TextBox Text=\"{{Binding {0} , Mode=TwoWay}}\" Grid.Row=\"{1}\" Grid.Column=\"{2}\" />\n";
            string CheckBoxTemplate = "<CheckBox IsChecked=\"{{Binding {0},Mode=TwoWay}}\" Grid.Row=\"{1}\" Grid.Column=\"{2}\" />";
            string ButtonTemplate = "<Button Content=\"{0}\" Grid.Row=\"{1}\" Grid.Column=\"{2}\" /> ";
            string VirtualNoBindingTextBox = "<TextBox  Grid.Row=\"{1}\" Grid.Column=\"{2}\"  />\n";

            string ComboBox2Template = "<telerik:DataFormComboBoxField Label=\"{0}\"\n" +
            ...
            if (IsTwoColumns)
            {
                ...
                foreach (var Item in MetaDatas)
                {
                    if (Item.IsVirtual == true)
                    {
                        ...
                        {
                            columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, 0) +
                                string.Format(ButtonTemplate, Item.PropertyName, GridRow, 1);
                        }
                        else
                        {

                            columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, 0) +
                                string.Format(VirtualNoBindingTextBox, Item.PropertyName, GridRow, 1);
                        }
                    }
                    else if (Item.IsList == true)
                    {
                        if (Item.IsForeignKey == true) //load the list from foregin key entity
                        {
                            ...
                            column = string.Format(ComboBox3Template, entity.EntityKey, Item.DisplayMemberName, GridRow, 1, Item.PropertyName, DataSourceName);
                    ...
                    else
                    {
                        switch (Item.TypeUsageName)
                        {
                            ...
                            case "int":
                                columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, 0) +
                                string.Format(TextBoxTemplate, Item.PropertyName, GridRow, 1);
                                break;
                            ...
                            case "bool":
                                columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, 0) +
                                string.Format(CheckBoxTemplate, Item.PropertyName, GridRow, 1);
                                break;
            ...
            else
            {
                ...
                foreach (var Item in MetaDatas)
                {
                    if (Item.IsVirtual == true)
                    {
                        if (Item.IsButton == true)
                        {
                            columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, ++GridColumn) +
                                string.Format(ButtonTemplate, Item.PropertyName, GridRow, ++GridColumn);
                        }
                        else
                        {

                            columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, ++GridColumn) +
                                string.Format(VirtualNoBindingTextBox, Item.PropertyName, GridRow, ++GridColumn);
                        }
                    }
                    else if (Item.IsList == true)
                    {
                        if (Item.IsForeignKey == true) //load the list from foregin key entity
                        {
                            ...
                            column = string.Format(ComboBox3Template, entity.EntityKey, Item.DisplayMemberName, GridRow, ++GridColumn, Item.PropertyName, DataSourceName);
                            //column = string.Format(ComboBox2Template , Item.Translation, Item.PropertyName, Item.Translation, DataSourceName, entity.EntityKey, Item.DisplayMemberName);
                            return column;
                        }
                        else // load the list from the referenced entity name
                    ...
                    else
                    {
                        switch (Item.TypeUsageName)
                        {
                            ...
                            case "int":
                                columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, ++GridColumn) +
                                string.Format(TextBoxTemplate, Item.PropertyName, GridRow, ++GridColumn);
                                break;
                            ...
                            case "bool":
                                columns += string.Format(TextBlockTemplate, Item.Translation, GridRow, ++GridColumn) +
                                string.Format(CheckBoxTemplate, Item.PropertyName, GridRow, ++GridColumn);
                                break;
                    ...
                    else if (Item.IsList == true)
                    {
                        if (Item.IsForeignKey == true) //load the list from foregin key entity
                        {
                            ...
                            column = string.Format(TextBlockTemplate, Item.Translation, GridRow, 0, string.Format(VisibilitySecurityTemplate, Item.PropertyName)) +
                                string.Format(ComboBox3Template, SelectedValueMemberPath, Item.DisplayMemberName, GridRow, 1, Item.PropertyName, DataSourceName, string.Format(DataFormSecurityTemplate, Item.PropertyName));
                            //column = string.Format(ComboBox2Template , Item.Translation, Item.PropertyName, Item.Translation, DataSourceName, entity.EntityKey, Item.DisplayMemberName);
                            // return column;
                            columns += column;
                        }
                    ...
                    else if (Item.IsList == true)
                    {
                        if (Item.IsForeignKey == true) //load the list from foregin key entity
                        {
                            ...
                            column = string.Format(TextBlockTemplate, Item.Translation, GridRow, ++GridColumn, string.Format(VisibilitySecurityTemplate, Item.PropertyName)) +
                                string.Format(ComboBox3Template, entity.EntityKey, Item.DisplayMemberName, GridRow, ++GridColumn, Item.PropertyName, DataSourceName, string.Format(DataFormSecurityTemplate, Item.PropertyName));
                            //column = string.Format(ComboBox2Template , Item.Translation, Item.PropertyName, Item.Translation, DataSourceName, entity.EntityKey, Item.DisplayMemberName);
                           // return column;
                            columns += column;
                        }
